/**
 * reporting-events.spec.js
 *
 * Captures a full-page screenshot of the Reporting > Event logs page.
 *
 * Requires:
...
const outputPath = resolve(__dirname, '../../output/web/adminconsole/reporting-events.png');

const orgId = process.env.ORG_ID || '';
const baseURL = process.env.WEB_APP_URL || 'https://vault.bitwarden.com';

test('reporting events - full page screenshot', async ({ page }) => {
  ...
  // The date filter form is rendered immediately once the component loads.
  // We then wait for either the events table or the "no events" message to confirm
  // the data has also finished loading.
